...
const TourContextProvider = (props) => {
  ...
  const taskMarkCancellationReceiptSent = async (bookingId) => {
    try {
      console.log("Calling cancellation receipt API for booking:", bookingId);
      const { data } = await axios.put(
      ...
      console.log("Cancellation receipt API success:", data);
      if (data.success) {
        setAllBookings((prev) =>
          prev.map((b) =>
            b._id === bookingId
  ...
  const taskMarkManageBookingReceiptSent = async (bookingId) => {
    try {
      console.log("Calling manage booking receipt API for booking:", bookingId);
      const { data } = await axios.put(
      ...
      console.log("Manage booking receipt API success:", data);
      if (data.success) {
        setAllBookings((prev) =>
          prev.map((b) =>
            b._id === bookingId
