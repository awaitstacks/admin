...
const TourAdminContextProvider = (props) => {
  ...
  // ─── Analytics functions ───────────────────────────────────────────
  //
  // ALL functions below now forward BOTH the legacy single year/month
  // params AND the new range params (fromYear/toYear/fromMonth/toMonth).
  // Previously these destructured ONLY { year, month, tourId } etc, so
  // range keys sent from Analytics.jsx were silently dropped before
  // reaching axios — the backend never saw them, fell through to its
  // default/unfiltered path, and produced wrong totals (e.g. an
  // "Average travellers" card showing 35 instead of the correct
  // range-restricted value).

  // 1. Summary — stat cards
  // GET /api/touradmin/analytics-summary?year=&month=&tourId=
  //                                     &fromYear=&toYear=&fromMonth=&toMonth=&tourId=
  const getAnalyticsSummaryData = useCallback(
    async ({ year = "", month = "", tourId = "", fromYear = "", toYear = "", fromMonth = "", toMonth = "" } = {}) => {
      try {
        const params = new URLSearchParams({
          ...(year && { year }),
          ...(month && { month }),
  ...
  // 2. Year-wise travellers — bar chart
  // GET /api/touradmin/analytics-year-wise
  //                       ?fromYear=&toYear=   (optional range restriction)
  const getAnalyticsYearWise = useCallback(
    async ({ fromYear = "", toYear = "" } = {}) => {
      try {
        ...
        const { data } = await axios.get(
          `${backendUrl}/api/touradmin/analytics-year-wise${params ? `?${params}` : ""}`,
          { headers: { aToken } },
        );
        if (data.success) {
          setAnalyticsYearWise(data.data || []);
  ...
  // 3. Month-wise travellers — line chart
  // GET /api/touradmin/analytics-month-wise?year=2026
  //                                        &fromYear=&toYear=&fromMonth=&toMonth=  (range mode)
  // NOTE: signature changed from (year) to (year, rangeOpts) — existing
  // call sites that only pass a single year value keep working exactly
  // as before since rangeOpts defaults to {} and contributes no params.
  const getAnalyticsMonthWise = useCallback(
    async (year, rangeOpts = {}) => {
      try {
        const { fromYear = "", toYear = "", fromMonth = "", toMonth = "" } = rangeOpts || {};
        const hasRange = fromYear || toYear || fromMonth || toMonth;
        // Normalize year explicitly: treat null/undefined the same way,
        // and only fall back to the current year when NOT in range mode.
        // (Default params alone don't catch `null`, only `undefined` —
        // this explicit check covers both.)
        const safeYear = (year === null || year === undefined)
          ? (hasRange ? null : new Date().getFullYear())
          : year;

        const params = new URLSearchParams({
          ...(!hasRange && safeYear && { year: safeYear }),
          ...(fromYear && { fromYear }),
          ...(toYear && { toYear }),
          ...(fromMonth && { fromMonth }),
          ...(toMonth && { toMonth }),
        ...
        const { data } = await axios.get(
          `${backendUrl}/api/touradmin/analytics-month-wise?${params}`,
          { headers: { aToken } },
        );
        if (data.success) {
          setAnalyticsMonthWise(data.data || []);
  ...
  // 4. Cancellation pool — bar/line chart
  // GET /api/touradmin/analytics-cancellation?view=year|month&year=2026
  //                                          &fromYear=&toYear=&fromMonth=&toMonth=  (range mode)
  const getAnalyticsCancellation = useCallback(
    async ({ view = "year", year, fromYear = "", toYear = "", fromMonth = "", toMonth = "" } = {}) => {
      try {
        const params = { view };
        if (year) params.year = year;
  ...
  // 5. Tour list — table
  // GET /api/touradmin/analytics-tour-list?year=&month=&type=&status=&tourId=
  //                                       &fromYear=&toYear=&fromMonth=&toMonth=&type=&status=&tourId=
  const getAnalyticsTourList = useCallback(
    async ({ year = "", month = "", type = "", status = "", tourId = "", fromYear = "", toYear = "", fromMonth = "", toMonth = "" } = {}) => {
      setAnalyticsLoading(true);
      setAnalyticsError(null);
      try {
        const params = new URLSearchParams({
  ...
  // 6. Tour name search — starts-with dropdown
  // GET /api/touradmin/analytics-search?q=K&year=2026
  //                                    &q=K&fromYear=&toYear=&fromMonth=&toMonth=  (range mode)
  const searchAnalyticsTours = useCallback(
    async ({ q = "", year = "", month = "", fromYear = "", toYear = "", fromMonth = "", toMonth = "" } = {}) => {
      if (!q) return { success: true, data: [] };
      try {
        const params = new URLSearchParams({
          q: q.toUpperCase(),
