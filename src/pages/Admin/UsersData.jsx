...
export default function UsersData() {
  ...
  // ── Same pattern as Analytics.jsx ─────────────────────────────
  // window.innerWidth listener → showTable boolean
  // sidebar (w-72 = 288px) appears at lg = 1024px
  // table needs >= 680px content area to be comfortable
  ...
  const showTable = contentWidth >= 920;
  ...
  return (
    <div style={{ padding: "20px 16px", background: "#f1f5f9", minHeight: "100vh", fontFamily: "inherit", boxSizing: "border-box", maxWidth: "100%", overflowX: "hidden" }}>
      ...
      {loading ? (
        ...
        <>
          {/* ── TABLE (showTable = contentWidth >= 680) ── */}
          {showTable && (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 500, tableLayout: "fixed" }}>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th style={thStyle}>Gender</th>
                      <th style={thStyle}>Address</th>
                      <th style={{ ...thStyle, textAlign: "right" }}>Joined On</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map((user, idx) => (
                      <tr key={user._id}
                        ...
                        <td style={{ ...tdStyle, overflow: "hidden" }}>
                          <div style={{ fontWeight: 600, color: "#1e293b", fontSize: 14, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name || "—"}</div>
                          <div style={{ fontSize: 12, color: "#64748b", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.email}</div>
                        </td>
                        ...
                        <td style={{ ...tdStyle, overflow: "hidden" }}>
                          {user.address?.line1 || user.address?.line2
                            ? <span style={{ fontSize: 12, color: "#475569", display: "block", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{[user.address.line1, user.address.line2].filter(Boolean).join(", ")}</span>
                            : <span style={{ color: "#cbd5e1", fontSize: 12 }}>—</span>}
                        </td>
              ...
              {filtered.map((user, idx) => (
                <div key={user._id} style={{ background: "#fff", borderRadius: 14, boxShadow: "0 1px 4px rgba(0,0,0,.06)", overflow: "hidden" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: 700, fontSize: 15, color: "#1e293b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.name || "Unknown"}</div>
                      <div style={{ fontSize: 12, color: "#64748b", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{user.email}</div>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      <div style={{ fontSize: 11, fontWeight: 600, color: "#475569", whiteSpace: "nowrap" }}>{formatDate(user.createdAt)}</div>
                  ...
                  <div style={{ padding: "10px 16px 14px", display: "flex", flexDirection: "column", gap: 8 }}>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                      <span style={{ background: "#f1f5f9", borderRadius: 8, padding: "4px 10px", fontSize: 12, color: "#475569", whiteSpace: "nowrap" }}>
                        📞 {user.phone || "No phone"}
                      </span>
                      <GenderBadge gender={user.gender} />
                    </div>
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 4, fontSize: 12, color: "#475569" }}>
                      <span style={{ flexShrink: 0 }}>📍</span>
                      <span>{user.address?.line1 || user.address?.line2
                        ? [user.address.line1, user.address.line2].filter(Boolean).join(", ")
                        : <span style={{ color: "#cbd5e1" }}>No address</span>}
                      </span>
                    </div>
