...
//   @media (max-width: 1023px) { .an-chart-grid { grid-template-columns: 1fr !important; } }

//   /* an-desk-table/an-mob-cards display switching is now handled in JS via
//      the showTable boolean (ResizeObserver-driven), not CSS media queries.
//      This avoids the table's fixed 760px minWidth ever existing in the DOM
//      when the card-list view is active. */

//   .an-sel { flex: 1 1 90px; min-width: 0; max-width: 100%; box-sizing: border-box; }
//   @media (max-width: 699px) { .an-sel { flex: 1 1 calc(50% - 6px) !important; } }
//   .an-search-wrap { flex: 1 1 180px; min-width: 0; max-width: 100%; box-sizing: border-box; }
//   @media (max-width: 699px) { .an-search-wrap { flex: 1 1 100% !important; } }
...
//   fbar: { background: "#fff", borderRadius: 12, padding: "10px 12px", marginBottom: 10, display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", boxShadow: "0 1px 4px rgba(0,0,0,.04)", minWidth: 0, maxWidth: "100%", boxSizing: "border-box" },
...
//   // Uses ResizeObserver on the Analytics root container (not window.innerWidth),
//   // so it automatically reflects the *actual available space* after the
//   // sidebar has taken its share — no race condition with the sidebar's own
//   // CSS transition, and no stale-layout-cache glitch from a fixed-minWidth
//   // table being toggled via CSS display:none. The table/card-list choice is
//   // rendered conditionally in JS (DOM removal), so the 760px table minWidth
//   // never exists in the DOM when the card-list view is active.
...
//   }, []);
//   // Table needs >= 760px to render without its own internal scroll feeling cramped;
//   // below that, show the card-expand list instead. null (not yet measured) defaults
//   // to showing the table to avoid a flash of card-list on first paint on wide screens.
//   const showTable = containerWidth === null || containerWidth >= 760;
...
//           ticks: { stepSize: 1, callback: (v) => ywLabels[Math.round(v)] ?? "", font: { size: 9 } },
//           grid: { display: false }
//         },
...
//           ticks: { stepSize: 1, callback: (v) => mwLabels[Math.round(v)] ?? "", font: { size: 9 } },
//           grid: { display: false }
//         },
//         y: { beginAtZero: true, ticks: { font: { size: 9 } }, grid: { color: "rgba(0,0,0,0.05)" } }
//       }
...
//       <Section title="Travellers" cols={7}>
...
//         </select>
//         <div className="an-search-wrap" style={{ position: "relative" }} ref={dropRef}>
...
//                     ].map(([lbl, val, color]) => (
//                       <div key={lbl} style={{ background: "#fff", borderRadius: 8, padding: "6px 8px", minWidth: 0 }}>
//                         <div style={{ fontSize: 9, color: "#aaa", marginBottom: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{lbl}</div>
//                         <div style={{ fontSize: 13, fontWeight: 700, color, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{val}</div>
//                       </div>
//                     ))}
//                   </div>
//                 </>
...
//                     <td style={{ ...S.td, textAlign: "center" }}><span style={S.pill("#dc2626", "#fef2f2", "#fca5a5")}>{totCancelledTrav}</span></td>
    ...
    <div ref={rootRef} className="an-root" style={S.pg}>
      ...
      <div style={{ ...S.fbar, marginBottom: 12 }}>
        <div className="an-fbar-grid">
          ...
          <button style={{ ...S.clr, width: "50%", marginLeft: "auto", display: "block" }} onClick={clearAll}>✕ Clear</button>        </div>
      </div>
      ...
      <Section title="Tour Bookings" cols={3}>
        <StatCard icon="📋" iconBg="#e0f2fe" label="Total TNR's" value={summary.totalBookings || 0} />
        <StatCard icon="⏳" iconBg="#f1f5f9" label="Unverified" value={summary.unverifiedBookings || 0} />
      ...
      <Section title="Travellers" cols={3}>
        <StatCard icon="👥" iconBg="#ede9fe" label="Total travellers" value={(summary.totalTravellers || 0).toLocaleString("en-IN")} />
        <StatCard icon="📊" iconBg="#e0e7ff" label="Average travellers (Only Active)" value={avgTravellers} />
        <StatCard icon="⏳" iconBg="#fef3c7" label="Unverified travellers" value={summary.unverifiedTravellers || 0} />
        <StatCard icon="🟢" iconBg="#dcfce7" label="Active travellers" value={summary.activeTravellers || 0} />
        <StatCard icon="🔶" iconBg="#fef3c7" label="Cancelled" value={summary.cancelledTravellers || 0} />
        <StatCard icon="🕐" iconBg="#fde8d8" label="Cancel request" value={summary.cancellationRequestTravellers || 0} />
