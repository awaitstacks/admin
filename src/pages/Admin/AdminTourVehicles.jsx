...
const SeatItem = ({ label, isLeader, positionInfo }) => (
  <div
    className={`flex flex-col items-center justify-center rounded-md sm:rounded-lg border-[1.5px] sm:border-2 shadow-sm transition-all 
      ...
      ${isLeader ? "bg-[#A32E25] text-white border-[#80241D]" : "bg-white border-slate-200 text-slate-700"}`}
  >
  ...
  return (
    <>
      ...
      <div className="p-4 sm:p-8 max-w-7xl mx-auto font-sans">
        ...
        ) : tourVehicles.length === 0 ? (
          <div className="py-20 text-center bg-white rounded-3xl shadow-sm border border-slate-200 max-w-3xl mx-auto">
            <Bus size={80} className="mx-auto text-slate-300 mb-6" />
            <h3 className="text-2xl font-bold text-slate-700 mb-3">
              No Vehicles Created for This Tour
            </h3>
            <p className="text-slate-500 mb-8 max-w-md mx-auto">
              You need at least one vehicle to enable seat allocation and
              proceed with bookings.
            </p>
            <button
              onClick={openCreateModal}
              className="inline-flex items-center gap-3 px-8 py-4 bg-indigo-600 text-white font-black text-lg rounded-2xl hover:bg-indigo-700 transition shadow-lg shadow-indigo-200"
            >
              <Plus size={24} />
              Add First Vehicle
            </button>
